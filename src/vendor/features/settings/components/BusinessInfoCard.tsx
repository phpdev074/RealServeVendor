...
export function BusinessInfoCard({ vendor }: BusinessInfoCardProps) {
  ...
  const handleToggleArea = (area: string) => {
    ...
    if (!allowedAreas.includes(area)) {
      toast.error('This area is outside your service zone. Contact RealServ support to expand your zone.');
      return;
    }
                    ...
                    <div className="grid grid-cols-2 gap-2.5">
                      {restrictedFilteredAreas.map(area => (
                        <div 
                          ...
                          onClick={() => handleToggleArea(area)}
                          title="Contact RealServ support to serve this area"
                        >
                          <Checkbox
                            id={`area-${area}`}
                            checked={false}
